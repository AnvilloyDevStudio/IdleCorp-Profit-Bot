    ...
    execute(message, args) {
        ...
        if (!guide) {
            ...
            for (const a of p) {
                ...
                if ((ind % 6) === 0 && p.slice(ind).length > 0)
                    embeds.push((p.slice(ind).length > 6) ? new Discord.MessageEmbed().setColor([200, 225, 255]) : new Discord.MessageEmbed().setColor([200, 225, 255]).setTimestamp());
                ind++;
            }
        ...
        else if (["versions", "vers", "vs"].includes(guide))
            return message.channel.send("Current guide version: `" + ncgl.guide.version + "`\nCurrent template version: `" + setting.ncguide.tempVer + "`\nCurrent structure version: `" + ncgl.guide.structure_version + "`");
        ...
        if (!ncgl.guide.pages.map(a => a.name).includes(arg))
            return message.channel.send("`EN0002`: The part was invalid.");
        ...
        if (num !== null && arg !== "thanks") {
            ...
            if (num > ctn.content[1].length || num < 0)
                return message.channel.send("`EN0002`: The number is out of range.");
        ...
        for (let a = 0; a < res.length; a++)
            embed.addField((!a) ? StringHandlers.capitalize(arg) : "\u200b", res[a], false);
        message.channel.send({ embeds: [embed] });
    }
};
export { Command };
//# sourceMappingURL=ncguide.js.map