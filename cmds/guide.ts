...
const Command: import("../types/command").Command = {
    ...
    execute(message, args) {
        ...
        if (level === "advanced") args.shift();
        if (!args.length) return message.channel.send("`EN0001`: Missing guide topic.");
        ...
        if (!(args[0] in guidelist[level])) return message.channel.send("`EN0002`: The guide topic was invalid.");
        const name = args.shift();
        const part = (!Object.keys(guidelist[level][name]).includes(args[0]))? Object.entries(guidelist.aliases[level][name]).find((a: [string, string[]]) => a[1].includes(args[0]))?.[0]: args[0];
        if (part === undefined&&args.length) return message.channel.send("`EN0002`: Invalid guide section.")
        ...
        if (!alias) {
            ...
            if (part) {
                ...
                embed.setTitle("Guide Aliases -- "+StringHandlers.capitalize(name))
                    .setColor([136, 136, 255])
                    .setDescription(Object.entries(guidelist["aliases"][level][name]).reduce((a, b: [string, string[]]) => a+StringHandlers.capitalize(b[0])+" -- Aliases: "+(b[1].join(", ") || "None")+"\n",  ""))
                    .addField("\u200b", "Version: v."+guidelist["version"])
                    .setFooter({text: message.client.user.username+" | "+setting.version, iconURL: message.client.user.displayAvatarURL()});
                message.channel.send({embeds: [embed]})
            }
