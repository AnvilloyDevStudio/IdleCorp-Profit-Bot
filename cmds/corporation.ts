...
const Command: import("../types/command").Command = {
    ...
    execute(message, args, extra) {
        ...
        let arg = args.join(" ");
        if (arg&&!["regions", "reg", "season", "season pass", "seasonpass", "pass", "challenges", "challenge"].includes(arg)) return message.channel.send("`EN0002`: Invalid corporation information.")
                            ...
                            if (!arg) {
                                ...
                                if (!id) {
                                    ...
                                    if (region["fromDb"]) {
                                        ...
                                        embed.addFields([
                                            {name: "Production", value: Object.entries(region["region"]["productionCoeffs"]).map((a: [string, number]) => "**"+a[0].replaceAll("_", " ")+"** | "+prod(a[1])).join("\n")},
                                            {name: "Active policy", value: region["region"]["activePolicies"].map(a => StringHandlers.capitalize(a["id"].replaceAll("_", " "))).join(", ")},
                                            {name: "Services", value: Object.entries(region["region"]["servicesProgress"]).map((a: [string, number]) => "**"+StringHandlers.capitalize(a[0].replaceAll("_", " "))+"** | "+a[1]+` (${new Decimal(a[1]).div(pol[a[0]]).mul(100).toFixed(2)}%)`)}
                                        ])
                                    } else {
                        ...
                        if (!id) {
                            ...
                            if (region["fromDb"]) {
                                ...
                                embed.addFields([
                                    ...
                                    {name: "Services", value: Object.entries(region["region"]["servicesProgress"]).map((a: [string, number]) => "**"+StringHandlers.capitalize(a[0].replaceAll("_", " "))+"** | "+NumberHandlers.numberToLocaleString(a[1])+` (${new Decimal(a[1]).div(pol[a[0]]).mul(100).toFixed(2)}%)`).join("\n")||"*No progress*"}
                                ])
                            } else {
                                embed.setDescription("No data.")
                            }
