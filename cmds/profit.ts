...
const Command: import("../types/command").Command = {
    name: "profit",
    aliases: ["pf"],
    syntax: "profit [--region:<region ID>] [--<time unit>] <facility> [<amount>]",
    description: "Calculating the profit of the facilities.",
    args: [
    ...
    manual: {
        description: "This command is used to calculate the profit of the specific facility (at a specific amount).",
        examples: [
            "profit steel mill",
    ...
    execute(message, args, extra) {
        ...
        if (!(fac in ICDetails["facilities"])) return message.channel.send("`EN0002`: Invalid facility: "+args.join(" ")+".");
        num *= {"day": 60*60*24, "hour": 60*60, "minute": 60}[unit]??1;
        const facDetail: IdleCorpTypes.FacilityDetailsNoArray = fac!=="airport"? Object.assign({}, ICDetails.facilities[fac]): {...ICDetails.facilities.airport, speed: 20};
                ...
                return new Promise<void>(rs => message.channel.send({embeds: [new Discord.MessageEmbed().setTitle("Discord account authorization").setURL(APIListener.DiscordAuthorization.authString+state)]}).then(msg => {
                    ...
                    listener.on("authorized", auth => {
                        IdleCorpConnection.getRegionalModifiers(auth.token, regionId, fac).then(b => {
                            ...
                            else facDetail.speed = Math.round(new Decimal(b).mul(facDetail.speed).toNumber());
                            rs();
                        })
                        listener.removeAllListeners();
                    })
        ...
        modpro.then(() => {
            ...
            if (sol[0] !== null) {
                for (let [a, b] of Object.entries(sol[0])) {
                    const bb = b.split(".");
                    c.push("**"+StringHandlers.capitalize(a).replaceAll("_", " ")+"** | $"+NumberHandlers.numberToLocaleString(parseInt(bb[0]))+((bb[1])? "."+bb[1]: ".00"));
                }
            } else c.push("*None*");
            for (let [a, b] of Object.entries(sol[1])) {
                const bb = b.split(".");
                p.push("**"+StringHandlers.capitalize(a).replaceAll("_", " ")+"** | $"+NumberHandlers.numberToLocaleString(parseInt(bb[0]))+((bb[1])? "."+bb[1]: ".00"));
            }
            const embed = new Discord.MessageEmbed()
                ...
                .addFields([{name: "Consumes", value: c.join("\n"), inline: false},
                    {name: "Produces", value: p.join("\n"), inline: false},
                    {name: "Profit", value: sol[2], inline: false},
