    ...
    execute(message, args, extra) {
        ...
        if (!args.length) {
            embed.setTitle("Help -- Commands")
                ...
                .setColor("GREEN")
                .setTimestamp();
            let res: Record<string, import("../types/command").Command[]> = {};
            ...
            for (const a of cates) res[a[0]] = [];
            res["Other"] = [];
            for (const cmd of extra.commands.values()) {
                const cate = cates.find(a => a[1].includes(cmd.name))?.[0] || "Other";
                if (cmd.perm>0&&Checks.isAdmin(message.member)) continue;
                res[cate].push(cmd);
            }
            for (const a in res) embed.addField("**"+StringHandlers.capitalize(a)+"**", res[a].map(a => "**"+a.name+"**\nDescription: "+a.description+"\nSyntax: "+a.syntax).join("\n\n"), false);
        } else {
            if (["--info", "--if"].includes(args[0])) {
                args.shift();
                if (!args.length) return message.channel.send("`EN0001`: Missing information header.");
                const info = args.join(" ");
                let infoctn = Object.keys(cmdInfo.aliases).find((a: keyof typeof cmdInfo.aliases) => cmdInfo.aliases[a].includes(info)) ?? info;
                ...
                if (ifDetail) args.shift();
                if (!args.length) return message.channel.send("`EN0001`: Missing command name.");
                let cmdName: [import("../types/command").Command, ...import("../types/command").Subcommand[]] = [extra.commands.get(args[0]) || [...extra.commands.values()].find(a => a.aliases.includes(args[0]))];
                if (cmdName[0]===undefined) return message.channel.send("`EN0002`: Invalid command `"+args[0]+"`.");
                let currentCmd: import("../types/command").Subcommand;
                for (let i in args.slice(1)) {
                    const subAliases = cmdName[cmdName.length-1].subcommands;
                    if (subAliases===undefined) return message.channel.send("`EN0002`: There is no subcommand in command `"+cmdName.map(a => a.name).join(" ")+"`.");
                    currentCmd = StringHandlers.findCmd(args[Number(i)+1], subAliases)
                    if (currentCmd===undefined) return message.channel.send(`\`EN0002\`: Invalid subcommand ${args[Number(i)+1]} in command ${cmdName.map(a => a.name).join(" ")}`);
