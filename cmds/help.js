    ...
    execute(message, args) {
        if (!args.length) {
            ...
            if (!["--info", "--if"].includes(args[0])) {
                ...
                if ((["reload", "send", "task", "idea"].includes(args[0])||(args[0] === "suggest"&&["change", "remove"].includes(args[1])))&&!message.member.roles.cache.findKey((a, b) => ["841622372351344650", "801052590389329930", "801052697498746920"].includes(b))) return message.channel.send("`EN0111`: You don't have enough permission to see the details of the command.");
