    ...
    execute(message, args) {
        ...
        if (args.length) {
            ...
            if (args[0].startsWith("--")) {
                ...
                if (version in ruledt)
                    ru = ruledt[version];
                else
                    return message.channel.send("`EN0012`: Invalid rule version.");
            }
            else
                version = Object.keys(ruledt).slice(-1)[0], ru = ruledt[version];
            if (args.length > 1)
                return message.channel.send("`EN0013`: Too many parameters.");
            let num = 0;
            if (args.length) {
                if (Number.parseInt(args[0]))
                    num = +args[0];
                else
                    return message.channel.send("`EN0002`: The rule number was invalid.");
            }
            if (!num) {
                let prints = [];
                for (const a in ru) {
                    ...
                    if (Array.isArray(ru[a])) {
                        if (a === "head_notes")
                            prints.push(...ru[a].map(a => "*" + a + "*"));
                        else if (a === "rules")
                            prints.push(...ru[a].map((a, b) => `**${b}.**${a}`));
                        else
                            prints.push(...ru[a]);
                    }
                    else
                ...
                embed.setDescription("**" + num + ".**" + (ru["rules"]?.[num - 1] ?? "Unexist.") + "\n\nv." + version);
        }
        else {
            const ru = Object.entries(ruledt).slice(-1)[0], version = ru[0], rule = ru[1];
            let prints = [];
            for (const a in rule) {
                if (a === "end") {
                    prints.push(...rule[a]);
                ...
                if (Array.isArray(rule[a])) {
                    if (a === "head_notes")
                        prints.push(...rule[a].map(a => "*" + a + "*"));
                    else if (a === "rules")
                        prints.push(...rule[a].map((a, b) => `**${b}.**${a}`));
                    else
                        prints.push(...rule[a]);
                }
                else
