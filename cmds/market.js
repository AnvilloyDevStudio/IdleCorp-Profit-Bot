...
const Command = {
    ...
    execute(message, args, extra) {
        ...
        if (args.length) {
            ...
            if (!args.length && !subscribe)
                return message.channel.send("`EN0001`: Missing asset name.");
            if (subscribe !== null) {
                if (args.length) {
                    const arg = args.join(" ").split(/, ?/);
                    const asset = arg.map(arg => Object.entries(ICAlias.ic.assets).find(a => a[1].includes(arg))?.[0] ?? arg);
                    if (asset.some(a => !(a in ICAlias.ic.assets)))
                        return message.channel.send("`EN0002`: Invalid asset name: " + asset.filter(a => !(a in ICAlias.ic.assets)).join(", ") + ".");
                    return extra.database.query("SELECT subscription FROM market_sub WHERE userid = \'" + message.author.id + "\';").then(dt => {
                        const newV = !dt.rowCount;
                        const value = dt.rowCount ? dt.rows[0].subscription : [[], []];
                        if (sellOrBuy === 0) {
                            if (subscribe)
                                value[0].push(...asset);
                            else
                                value[0] = value[0].filter(a => !asset.includes(a));
                            if (subscribe)
                                value[1].push(...asset);
                            else
                                value[1] = value[1].filter(a => !asset.includes(a));
                        }
                        else if (sellOrBuy === 1) {
                            if (subscribe)
                                value[0].push(...asset);
                    ...
                    return extra.database.query("SELECT subscription FROM market_sub WHERE userid = \'" + message.author.id + "\';").then(dt => {
                        ...
                        if (sellOrBuy === 0) {
                            if (subscribe)
                                value[0].push(...Object.keys(ICAlias.ic.assets));
                            else
                                value[0] = [];
                            if (subscribe)
                                value[1].push(...Object.keys(ICAlias.ic.assets));
                            else
                                value[1] = [];
                        }
                        else if (sellOrBuy === 1) {
            ...
            if (!(asset in ICAlias.ic.assets))
                return message.channel.send("`EN0002`: Invalid asset name: " + asset + ".");
            else
                IdleCorpConnection.market.asset(asset).then(mk => {
                    if (mk === null)
                        return message.channel.send("`EN0131`: Missing market data with asset `" + asset + "`.");
                    ...
                    if (sellOrBuy === 0) {
                        const orders = [[], []];
                        for (const p in mk.buyOffers)
                            orders[0].push("> $" + new Decimal(p).div(100).toFixed(2) + " -- " + mk.buyOffers[p]);
                        for (const p in mk.sellOffers)
                            orders[1].push("> $" + new Decimal(p).div(100).toFixed(2) + " -- " + mk.sellOffers[p]);
                        embed.setTitle("Market -- " + StringHandlers.capitalize(asset.replaceAll("_", " "))).setDescription("**Buy Offers**\n" + (orders[0].join("\n") || "*No orders*") + "\n\n**Sell Offers**\n" + (orders[1].join("\n") || "*No orders*"));
                    }
                    else if (sellOrBuy === 1) {
                        const orders = [];
                        for (const p in mk.sellOffers)
                            orders.push("> $" + new Decimal(p).div(100).toFixed(2) + " -- " + mk.sellOffers[p]);
                        embed.setTitle("Market -- " + StringHandlers.capitalize(asset.replaceAll("_", " ")) + " Sell Offers").setDescription(orders.join("\n") || "*No orders*");
                    }
                    else {
                        const orders = [];
                        for (const p in mk.buyOffers)
                            orders.push("> $" + new Decimal(p).div(100).toFixed(2) + " -- " + mk.buyOffers[p]);
