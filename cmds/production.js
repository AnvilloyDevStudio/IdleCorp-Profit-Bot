...
const Command = {
    ...
    async execute(message, args, extra) {
        if (!args.length)
            return message.channel.send("`EN0001`: Missing parameter.");
        let regionId = null;
        ...
        if (["--to", "--until", "--goal"].includes(args[0].toLowerCase())) {
            args.shift();
            if (!args.length)
                return message.channel.send("`EN0021`: Missing production goal parameter.");
            let goal = Math.ceil(NumberHandlers.numalias(args.shift()));
            if (isNaN(goal))
                return message.channel.send("`EN0022`: The production goal was invalid.");
            let num = 1;
            ...
            if (!(fac in ICDetails["facilities"]))
                return message.channel.send("`EN0002`: Invalid facility name: " + args.join(" ") + ".");
            let faclist = { ...ICDetails["facilities"], airport: { ...ICDetails.facilities.airport, speed: 20 } }, faccs = faclist[fac]["consumes"], speed = faclist[fac]["speed"], modpro;
                            ...
                            listener.on("authorized", auth => {
                                IdleCorpConnection.getRegionalModifiers(auth.token, regionId, fac).then(b => {
                                    if (b === null)
                                        ...
                                        speed = /*Array.isArray(speed)? speed.map(a => Math.round(new Decimal(b).mul(a).toNumber())):*/ Math.round(new Decimal(b).mul(speed).toNumber());
                                    rs();
                                });
                                listener.removeAllListeners();
                            });
            ...
            if (!args.length)
                return message.channel.send("`EN0021`: Missing production parameter(s).");
            let mods = { technology: [], service: [], policy: [], happiness: 0 };
            let arg = "";
            let region = { happiness: 0, productionCoeffs: {}, policies: [], services: [] };
            if (regionId) {
                let auth = await APIListener.DiscordAuthorization.getAuthbyID(extra.database, message.author.id);
                if (!auth) {
                    const state = Date.now().toString() + "-" + message.author.id;
                    const listener = new APIListener.DiscordAuthorization(extra.database, state);
                    await new Promise(rs => message.channel.send({ embeds: [new Discord.MessageEmbed().setTitle("Discord account authorization").setURL(APIListener.DiscordAuthorization.authString + state)] }).then(msg => {
                        listener.on("timeout", () => {
                            msg.edit({ embeds: [new Discord.MessageEmbed().setTitle("Authorization Timeout").setDescription("Calculating without regional modifiers instead...").setColor("RED").setTimestamp()] });
                            listener.removeAllListeners();
                            rs();
                ...
                if (auth !== null)
                    await IdleCorpConnection.getRegionalModifiers(auth.token, regionId).then(r => {
                        ...
                        if (r.servicesProgress.park_large === 1000000000)
                            region.services.push("large_park");
                        if (r.servicesProgress.park_medium === 100000000)
                            region.services.push("medium_park");
                        if (r.servicesProgress.park_small === 10000000)
                            region.services.push("small_park");
                    });
            }
            ...
            if (args[0].startsWith("--")) {
                ...
                if (c.some(a => !["--technology", "--tech", "--service", "--serv", "--policy"].includes(a)))
                    return message.channel.send("`EN0022`: Invalid flag detected.");
                ...
                if (!a)
                    return message.channel.send("`EN0022`: Invalid items detected.");
                for (let t = 1; t < a.length; t++) {
                    switch (b[t - 1]) {
                        case "technology":
                            mods.technology = a[t].split(/, ?/).map(a => a.split(/: ?/)).map(a => [a[0], a[1] === undefined ? 1 : parseInt(a[1])]);
                            break;
                        case "service":
                            mods.service = a[t].split(/, ?/);
                            break;
                        case "policy":
                            mods.policy = a[t].split(/, ?/);
                            break;
                    }
                }
                if (mods["technology"].filter(a => !(a[0] in icals["ic"]["technologies"])).length)
                    return message.channel.send("`EN0022`: Invalid items detected (in technology).");
                if (mods["service"].filter(a => !(a[0] in icals["ic"]["services"])).length)
                    return message.channel.send("`EN0022`: Invalid items detected (in service).");
                if (mods["policy"].filter(a => !(a[0] in icals["ic"]["policies"])).length)
                    return message.channel.send("`EN0022`: Invalid items detected (in policy).");
                if (mods["technology"].some(a => isNaN(a[1])))
                    return message.channel.send("`EN0022`: Invalid number parameter detected, in technology parameter.");
                ...
                if (mods.technology.length) { // airport tram is ignored
                    if ("oil_mapping" in mods.technology)
                        Facilities.oil_well.produces.crude_oil = 14;
                    if ("oil_mapping_u" in mods.technology)
                        Facilities.oil_well.produces.crude_oil = 18;
                    if ("oil_mapping_uu" in mods.technology)
                        Facilities.oil_well.produces.crude_oil = 100;
                    if ("coal_detector" in mods.technology)
                        Facilities.coal_mine.produces.coal = 12;
                    if ("coal_detector_u" in mods.technology)
                        Facilities.coal_mine.produces.coal = 16;
                    if ("coal_detector_uu" in mods.technology)
                        Facilities.coal_mine.produces.coal = 80;
                    if ("advanced_woodworking" in mods.technology)
                        Facilities.furniture_factory.speed = 200;
                    if ("advanced_woodworking_u" in mods.technology)
                        Facilities.furniture_factory.speed = 160;
                    if ("advanced_woodworking_uu" in mods.technology)
                        Facilities.furniture_factory.speed = 30;
                    if ("bauxite_detector" in mods.technology)
                        Facilities.bauxite_mine.produces.bauxite = 50;
                    if ("bauxite_detector_u" in mods.technology)
                        Facilities.bauxite_mine.produces.bauxite = 60;
                    if ("vacuum_distillation" in mods.technology)
                        Facilities.oil_refinery.speed = 15;
                    if ("vacuum_distillation_u" in mods.technology)
                    ...
                    if ("gold_detector" in mods.technology)
                        Facilities.gold_mine.produces.gold = 6;
                    if ("gold_detector_u" in mods.technology)
                        Facilities.gold_mine.produces.gold = 8;
                    if ("gold_detector_uu" in mods.technology)
                        Facilities.gold_mine.produces.gold = 10;
                    ...
                    for (const [tech] of new Set(mods.technology)) {
                        switch (tech) {
                            ...
                            case "log_loader":
                                Facilities.tree_farm.speed -= 1;
                                break;
                        }
                    }
                }
                if (mods.service.length) {
                    for (const service of new Set(mods.service))
                        switch (service) {
                            case "small_park":
                                mods.happiness += 1;
                                break;
                ...
                if (mods.policy.length) {
                    const buildPriceIndices = Object.fromEntries(Object.keys(ICDetails.info.facilities).map(a => [a, 1]));
                    let energyPrice = 1;
                    let landPrice = 1;
                    for (const policy of new Set(mods.policy))
                        switch (policy) {
                            case "solar_subsidies":
                                Assets.energy *= 0.9;
                                buildPriceIndices.solar_power_plant -= 0.1;
                                break;
                            case "public_logistics_access":
                                for (const f in buildPriceIndices)
                                    buildPriceIndices[f] += 0.1;
                                break;
                            case "regional_planning":
                                for (const f in buildPriceIndices)
                                    buildPriceIndices[f] -= 0.25;
                                landPrice += 0.05;
                                break;
                            case "tourism_campaign":
                                Facilities.airport.consumes.jet_fuel += 4;
                                break;
                            case "low_income_tax":
                                mods.happiness += 2;
                                break;
                    ...
                    for (const f in buildPriceIndices)
                        ICDetails.info.facilities[f].construct.money *= Math.round(buildPriceIndices[f] * 100) / 100;
                }
                ...
                for (const coeff in region.productionCoeffs)
                    for (const f of productionMods[coeff])
                        Facilities[f]["speed"] *= region.productionCoeffs[coeff];
                for (const f in Facilities)
                    Facilities[f].speed = Math.round(Facilities[f].speed / ((100 + mods.happiness) / 100));
                if (Facilities.airport.speed < 20)
                    Facilities.airport.speed = 20;
            }
            const [facs, assets] = (arg ? arg : args.join(" ")).match(/^{([\w ]+: ?[0-9]+(?:, ?[\w ]+: ?[0-9]+)*)}(?: {([\w ]+: ?[0-9]+(?:, ?[\w ]+: ?[0-9]+)*)})?$/)?.slice(1) ?? [];
            if (!facs)
                return message.channel.send("`EN0022`: Invalid facilities and/or assets detected.");
            const Facs = facs.split(/, ?/).map(a => a.split(/: ?/)).map(a => [Object.entries(icals["ic"]["facilities"]).find(b => b[1]?.includes(a[0]))?.[0] ?? a[0].replaceAll(" ", "_"), parseInt(a[1]) ?? 1]);
            if (Facs.filter(a => !(a[0] in icals["ic"]["facilities"])).length)
                return message.channel.send("`EN0022`: Invalid facilities and/or assets detected.");
            if (!assets) {
                let production = Object.keys(Assets).map(a => [a, new Fraction(0)]);
                for (const [a, b] of Facs) {
                    const c = Facilities[a];
                    if (c["consumes"] !== null)
                        for (const d of Object.entries(c["consumes"])) {
                ...
                let Aass = assets.split(/, ?/).map(a => a.split(/: /)).map(a => [Object.entries(icals["ic"]["assets"]).find(b => b[1]?.includes(a[0]))?.[0] ?? a[0], parseInt(a[1]) ?? 1]);
                let production = Object.entries(Assets).map(a => [a[0], new Fraction(0)]);
                let consumes = [];
                if (Aass.some(a => !(a[0] in icals["ic"]["assets"])))
                    return message.channel.send("`EN0022`: Invalid asset(s) detected.");
                for (const [a, b] of Facs) {
                    const c = Facilities[a];
                    if (c["consumes"] !== null)
                        for (const d of Object.entries(c["consumes"])) {
                ...
                const cycle1 = cycle.map(a => [a[0], a[2]]);
                for (let a = 0, lows = 0; a < cycle1.length; a++) {
                    if (cycle1[lows][1] > cycle1[a][1])
                        lows = a;
                    low = cycle1[lows][1];
                }
                const cycle2 = [];
