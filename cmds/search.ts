...
const Command: import("../types/command").Command = {
    ...
    execute(message, args) {
        ...
        new Promise<object>((rs, rj) => https.request(host, (resp) => {
            let data = "";
            resp.on("data", (chunk) => {
                data += chunk;
            })
            ...
            if ("error" in d) return message.channel.send({embeds: [new Discord.MessageEmbed().setTitle("Error").setDescription(d["error"]["info"]).setFooter({text: message.client.user.username+" | "+setting.version, iconURL: message.client.user.displayAvatarURL()}).setColor([255, 85, 119])]});
            d = d["parse"];
            if (section) {
                ...
                const dd = d;
                return new Promise<object>((rs, rj) => https.request(host, (resp) => {
                    let data = "";
                    resp.on("data", (chunk) => {
                        data += chunk;
                    })
