...
const Command: import("../types/command").Command = {
    ...
    execute(message, args) {
        if (!Checks.isDev(message.member)) return message.channel.send("`EN0111`: You don't have enough permission.");
        const channels = message.guild.channels.cache;
        const arg = args.join("");
        const chid = arg.match(/\d+/)?.[0];
        if (!channels.has(chid)) return message.channel.send("`EN0006`: The channel id was invalid.");
        (<Discord.TextChannel>channels.get(chid)).send("This is a message")
    }
}
export {Command};