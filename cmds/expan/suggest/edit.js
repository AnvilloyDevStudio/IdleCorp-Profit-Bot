...
export default (message, args, extra) => {
    const suggest_id = parseInt(args.shift()), new_suggest = args.join(" ");
    if (Number.isNaN(suggest_id))
        return message.channel.send("`EN0002`: The number was invalid.");
    if (!new_suggest.length)
        return message.channel.send(`\`EN0001\`: Missing new suggestion.`);
    extra.database.query("SELECT userid FROM \"suggestions\" WHERE suggestion_id = " + suggest_id + ";").then(dt => {
        if (!dt.rowCount)
            return message.channel.send("`EN0002`: The suggestion id was invalid.");
        if (!Checks.isDev(message.member) && !(dt.rows[0].userid === message.author.id))
            return message.channel.send("`EN0122`: You were not the author of the suggestion.");
