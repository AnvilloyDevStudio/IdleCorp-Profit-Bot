...
export default (message: Discord.Message, args: import("../../../types/command").Input, extra: import("../../../types/command").extraArgs) => {
    ...
    const type: string = check[args[0]] ?? (args.join("_")||undefined);
    if (type&&!["in_review", "verified", "pass", "not_pass", "done"].includes(type)) return message.channel.send("`EN0002`: Invalid suggestion status detected.");
    extra.database.query<{suggestion_id: number, suggestions: string, status: string, date: Date, userid: string, votes: any[]}>("SELECT * FROM \"suggestions\";").then(dt => {
