...
export default (message: Discord.Message, args: import("../../../types/command").Input, extra: import("../../../types/command").extraArgs) => {
    if (!Checks.isDev(message.member)) return message.channel.send("`EN0111`: Missing required permission.");
    const alias = {"ir": "in_review", "i": "in_review", "v": "verified", "p": "pass", "np": "not_pass", "n": "not_pass", "d": "done"},
    suggest_id = parseInt(args.shift());
    ...
    extra.database.query<{status: string}>("SELECT status FROM \"BenChueng0422/IdleCorp-Profit\".\"suggestions\" WHERE suggestion_id = "+suggest_id).then(dt => {
        if (!dt.rowCount) return message.channel.send("`EN0006`: The suggestion id was invalid.");
        if (dt.rows[0].status === status) return message.channel.send("`EN0002`: The status of the suggestion was already `"+status+"`.");
