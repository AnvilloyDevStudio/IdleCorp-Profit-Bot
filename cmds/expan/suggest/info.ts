...
export default (message: Discord.Message, args: import("../../../types/command").Input, extra: import("../../../types/command").extraArgs) => {
    if (!args.length) return message.channel.send("`EN0001`: Missing suggestion id.");
    const suggest_id = parseInt(args.shift());
    if (Number.isNaN(suggest_id)) return message.channel.send("`EN0002`: The suggesion id was invalid.");
    extra.database.query<{suggestion_id: number, suggestions: string, status: string, date: Date, userid: string, votes: any[]}>("SELECT * FROM \"suggestions\" WHERE suggestion_id = "+suggest_id).then(dt => {
