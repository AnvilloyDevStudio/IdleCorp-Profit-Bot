...
export default (message: Discord.Message, args: import("../../../types/command").Input) => {
    const cds = cdslist.codes.update;
    if (!args.length) return message.channel.send("`EN0001`: Missing code or the code cannot be detected.");
    ...
    for (const a in cds) {
        if (code === a) {
            ...
            } else if (code[1] === "A" && !Checks.isAdmin(message.member)) return message.channel.send("`EN0112`: The code was required permission.");
