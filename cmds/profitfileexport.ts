...
import * as IdleCorpTypes from "../types/IdleCorpTypes";
    ...
    execute(message, args, extra) {
        ...
        if (regionId) modpro = APIListener.DiscordAuthorization.getAuthbyID(extra.database, message.author.id).then(auth => {
            if (!auth) {
                    ...
                    listener.on("authorized", auth => {
                        IdleCorpConnection.getRegionalModifiers(auth.token, regionId, null).then(b => {
                            if (b===null) msg.edit({embeds: [new Discord.MessageEmbed().setTitle("Invalid Region ID").setDescription("Calculating without regional modifiers instead...").setColor("RED").setTimestamp()]});
                                ...
                                for (const f in ICDetails.facilities) if (f in b) ICDetails[f].speed = Math.round(new Decimal(b[f]).mul(ICDetails[f].speed).div(100+b["happiness"]).toNumber());
                                    else ICDetails.facilities[f].speed = Math.round(new Decimal(ICDetails[f].speed).div(100+b["happiness"]).toNumber());
                            }
                            rs();
                        })
                        listener.removeAllListeners();
                ...
                return IdleCorpConnection.getRegionalModifiers(auth.token, regionId, null).then(b => {for (const f in ICDetails.facilities) if (f in b) ICDetails.facilities[f].speed = Math.round(new Decimal(b[f]).mul(ICDetails.facilities[f].speed).div(100+b["happiness"]).toNumber());
                    else ICDetails.facilities[f].speed = Math.round(new Decimal(ICDetails.facilities[f].speed).div(100+b["happiness"]).toNumber());});
            }
        })
        ...
        modpro.then(() => {
            for (const fac in ICDetails["facilities"]) {
                ...
                for (const a of [sol[0], sol[1], soland[1]]) {
                    ...
                    if (a !== null) {
                        ...
                        s2.push(s.join("\n"));
                    } else s2.push(version===2? "> *None*": "| None");
                    count++;
                }
                {
            ...
            if (version===1) {
                ...
                for (const a in Facilities) {
                    let stuff = [],
                    i = [0, 0];
                    for (const b of Object.entries<any>(Facilities[a])) {
                        if (["consumes", "produces"].includes(b[0])) {
                            const e = ["consumes", "produces"].indexOf(b[0]);
                            if (i[e] === 0) stuff.push(StringHandlers.capitalize(b[0]).replaceAll("_", " "));
                            if (b[1] === null) stuff.push("| None");
                            else {
                                for (const d in b[1]) {
                    ...
                    if (Array.isArray(ICDetails["facilities"][a]["speed"])) {
                        let speedar = [];
                        for (const b of ICDetails["facilities"][a]["speed"]) speedar.push(NumberHandlers.toTime(b).slice(1).map(a => (a.toString().length<2)? "0"+a: a).join(":"));
                        speedar.splice(1, 0, "to");
                        speed = speedar.join(" ");
                    } else speed = NumberHandlers.toTime(ICDetails["facilities"][a]["speed"]).slice(1).map(a => (a.toString().length<2)? "0"+a: a).join(":");
                ...
                for (const f in Facilities) {
                    ...
                    if (Array.isArray(ICDetails["facilities"][f]["speed"])) {
                        let speedarr = [];
                        for (const b of ICDetails["facilities"][f]["speed"]) speedarr.push(NumberHandlers.toTime(b).slice(1).map(a => (a.toString().length<2)? "0"+a: a).join(":"));
                        speedarr.splice(1, 0, "to");
                        speed = speedarr.join(" ");
                    } else speed = NumberHandlers.toTime(ICDetails["facilities"][f]["speed"]).slice(1).map(a => (a.toString().length<2)? "0"+a: a).join(":");
                ...
                archive.finalize().then(() => {
                    if (DM) message.author.createDM().then(channel => channel.send({content: "Zipped Profit Files", files: [new Discord.MessageAttachment(resf.getBuffer(), "IdleCorp Profit Files.zip")]}).then(() => message.channel.send("It has sent via your DM.")));
                    else message.channel.send({content: "Zipped Profit Files", files: [new Discord.MessageAttachment(resf.getBuffer(), "IdleCorp Profit Files.zip")]})
                })
            } else if (version===2) {
                let embed = [new Discord.MessageEmbed().setTitle("IdleCorp Profit Embed Full")],
                check = 0;
                ...
                if (DM) message.author.createDM().then(channel => channel.send({embeds: embed}).then(() => message.channel.send("It has sent via your DM.")));
                ...
                if (DM) message.author.createDM().then(channel => channel.send({content: "IdleCorp Profit profit.txt", files: [resf]}).then(() => message.channel.send("It has sent via your DM.")));
                else message.channel.send({content: "IdleCorp Profit profit.txt", files: [resf]});
            }
        })
    }
}
