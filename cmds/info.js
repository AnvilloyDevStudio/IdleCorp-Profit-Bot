...
const Command = {
    ...
    execute(message, args) {
        ...
        if (theme) {
            ...
            if (req === "all")
                return message.channel.send({ embeds: [embed.setTitle("Theme -- All themes").setDescription("Themes:\n" + Object.keys(icinfo.themes.themes).map(a => "> " + StringHandlers.capitalize(a)).join("\n"))] });
            else if (req === "aliases")
                return message.channel.send({ embeds: [embed.setTitle("Theme -- Aliases of all themes").setDescription("Theme aliases:\n" + Object.entries(icinfo.themes.themes).map(a => "> " + a[0] + " | " + (a[1].join("\n") || "None")))] });
            if (!(req in icinfo.themes.themes))
                return message.channel.send("`EN0002`: The theme name is invalid.");
        ...
        function findName(req, alias = true) {
            for (const a of Object.values(icinfo))
                if (req in a)
                    return req;
            if (alias)
                for (const [a, b] of Object.values(icals.ic).flatMap(a => Object.entries(a)))
                    if (b.includes(req))
                        return a;
            return undefined;
        }
        ...
        if (!reqsl)
            return message.channel.send(`\`EN0002\`: Input \"${req}\" is invalid, or it is just not exist.`);
        if (reqsl.endsWith("_u"))
            reqsl = reqsl.slice(0, -2), techlv = 1;
        else if (reqsl.endsWith("_uu"))
            reqsl = reqsl.slice(0, -3), techlv = 2;
        if (!findName(reqsl, false))
            return message.channel.send(`\`EN0131\`: Input \"${req}\" is not exist.`);
        const infoclass = Object.entries(icinfo).find(a => reqsl in a[1])[0];
        const infoObj = icinfo[infoclass][reqsl];
        ...
        if (infoclass === "assets") {
            ...
            if ("icp" in infoObj)
                embed.addField("More:", infoObj["icp"], false);
                        ...
                        (reqsl === "lamp") ? "[Wikipedia](https://en.wikipedia.org/wiki/light_fixture)" :
                            (reqsl === "ccd") ? "[Wikipedia](https://en.wikipedia.org/wiki/charge-coupled_device)" :
                                (reqsl === "hq") ? "[Wikipedia](https://en.wikipedia.org/wiki/Headquarters)" :
                                    (infoObj["wikipedia"] !== "None") ? "[Wikipedia](https://en.wikipedia.org/wiki/" + reqsl + "})" : ""), false);
        }
        else if (infoclass === "facilities") {
            embed.addFields([{ name: "IdleCorp Examine:", value: infoObj["icinfo"], inline: false },
                { name: "Construction materials:", value: Object.entries(infoObj["construct"]).map((a) => (a[0] === "money") ? "> $" + a[1].toLocaleString("en-US", { minimumFractionDigits: 2 }) : "> " + a[1].toLocaleString("en-US") + " **" + StringHandlers.capitalize(a[0]) + "**").join("\n"), inline: false },
                { name: "IdleCorp Wiki:", value: infoObj["icwiki"], inline: false },
                { name: "Wikipedia:", value: infoObj["wikipedia"], inline: false }]);
            if (reqsl in ICDetails["facilities"]) {
                const facpro = ICDetails["facilities"][reqsl];
                embed.addField("Production:", "**Consumes**\n" + ((facpro["consumes"] === "None") ? "> None\n" : Object.entries(facpro["consumes"]).map((a) => "> **" + (StringHandlers.capitalize(a[0])).replaceAll("_", " ") + "** | " + a[1].toLocaleString("en-US", { minimumFractionDigits: 2 }), []).join("\n")) + "\n**Produces**\n" + ((facpro["produces"] === "None") ? "> None\n" : Object.entries(facpro["produces"]).map((a) => "> **" + (StringHandlers.capitalize(a[0])).replaceAll("_", " ") + "** | " + a[1].toLocaleString("en-US", { minimumFractionDigits: 2 }), []).join("\n")) + "\nSpeed: " + ICDetails["facilities"][reqsl]["speed"].toString() + " seconds", false);
            }
            if ("icp" in infoObj)
                embed.addField("More:", infoObj["icp"], false);
            ...
            embed.addField("\u200b", ((icals["ic"]["facilities"][reqsl].length) ? "Aliases: " + icals["ic"]["facilities"][reqsl].join(", ") : "Aliases: None") + ((reqsl === "steel_mill" && Math.random() <= 0.02) ? "\n\n*I always typo this facility name as \"still mill\"*" : "") + ((reqsl === "furniture_factory") ? ((Math.random() <= 0.2) ? "\n\n*Why the IdleCorp Wiki page has so many words...*" : "") + ((Math.random() <= 0.1) ? "\n\n*Because the IdleCorp Wiki page has too many work, I had to change the layout of all...*" : "") : "") + ((reqsl.endsWith("factory") && Math.random() <= 0.05) ? "\n\n*I really want to know who made this facility page...*" : "") + ((reqsl === "research_chemical_factory" && Math.random() < (1 / 3)) ? "\n\n*Why this facility has so many aliases...*" : "") + "\n\n\n[IdleCorp Wiki](https://wiki.idlecorp.xyz/index.php/" + reqsl + ")" + ((infoObj["wikipedia"] !== "None") ? "\n[Wikipedia](https://en.wikipedia.org/wiki/" + reqsl + ")" : ""), false);
        }
            ...
            embed.addFields([{ name: "IdleCorp Examine:", value: infoObj["icinfo"] + "\n\nRarity: " + infoObj["rarity"] + "\n\nAll blueprints **cannot** trade and be sold in the market and the retail stores.", inline: false },
                { name: "Requires:", value: Object.entries(infoObj["require"]).map((a) => a[1].toLocaleString("en-US", { minimumFractionDigits: 2 }) + " **" + StringHandlers.capitalize(a[0]) + "**", []), inline: false },
                { name: "IdleCorp Wiki:", value: infoObj["icwiki"], inline: false }]);
            if ("icp" in infoObj)
                embed.addField("More:", infoObj["icp"], false);
            ...
            if (techlv && infoObj["upgrade"]["u".repeat(techlv)] === "None")
                return message.channel.send("`EN0004`: The technology has no \"" + "u".repeat(techlv) + "\" upgrade.");
            ...
            embed.addField("\u200b", ((icals["ic"]["services"][reqsl].length) ? "Aliases: " + icals["ic"]["services"][reqsl].join(", ") : "Aliases: None") + ((infoObj["wikipedia"] !== "None") ? "\n\n[Wikipedia](https://en.wikipedia.org/wiki/" + reqsl + ")" : ""), false);
        }
        else if (infoclass === "pollicies") {
            embed.addField("IdleCorp Examine:", "Affect: " + infoObj["affect"] + "\nCost: " + infoObj["cost"] + " *Funding points*", false);
            if ("icp" in infoObj)
                embed.addField("More:", infoObj["icp"], false);
            embed.addField("\u200b", ((icals["ic"]["policies"][reqsl].length) ? "Aliases: " + icals["ic"]["policies"][reqsl].join(", ") : "Aliases: None") + ((infoObj["wikipedia"] !== "None") ? "\n\n[Wikipedia](https://en.wikipedia.org/wiki/" + reqsl + ")" : ""), false);
        }
        else
            return message.channel.send("`EN0131`: The type of the input is not handled.");
        if (!img)
