        ...
        if (args.length) {
            ...
            if (args[0].startsWith("--")) {
                version = args.shift().slice(2),
                    ud = udslist[version];
                if (!ud)
                    return message.channel.send("`EN0012`: The version was invalid.");
            }
            else
                version = Object.keys(udslist).slice(-1)[0], ud = Object.values(udslist).slice(-1)[0];
            const mores = args.join(" ");
            if (mores) {
                let more = mores.split(".").map(a => a.replaceAll("_", " ")), fn = "", moo = [], b = ud;
                const first = Object.entries(udsals["first"]), second = Object.entries(udsals["second"]), third = Object.entries(udsals["third"]);
                for (const a in more) {
                    if (!Number(a)) {
                        if (!first.flatMap(a => a[1]).includes(more[a]))
                            more[a] = first.find(b => b[1].includes(more[a]))?.[0];
                    }
                    else if (!(second.flatMap(a => a[1]).includes(more[a]) || third.flatMap(a => a[1]).includes(more[a])))
                        more[a] = (more[a] !== "plans to update") ? second.find(b => b[1].includes(more[a]))?.[0] : third.find(b => b[1].includes(more[a]))?.[0];
                    b = b[more[a]];
                    if (!b)
                        return message.channel.send("`EN0002`: The argument is invalid or unexist in this version of update");
                    moo.push(more[a]);
                }
                more = moo;
                let store = b, prints;
                if (!(store["length"] || Object.keys(store).length))
                    prints = "\n**Nothing to show**";
                else {
                        ...
                        for (const a in store) {
                            if (more[0] === "commands") {
                                if (["adds", "changes", "deletes"].includes(a)) {
                                    ...
                                    if (a[0].match(/[A-Z]/))
                                        store2.push(+a + " -- " + store[a]);
                                    else
                                        store2.push("`" + a + "` -- " + store[a]);
                                }
                            }
                            else if (more[0] === "other") {
                                if (store[a])
                                    store2.push("\"" + a + "\" -- " + store[a]);
                            }
                                ...
                                if (a in ud["plans to update"]) {
                                    if (store[a])
                                        ...
                                        store2.push("**" + StringHandlers.capitalize(a) + ":**", ...Object.entries(store[a]).map((a) => "\"" + StringHandlers.capitalize(a[0]) + "\" -- " + ((a[1].startsWith("v")) ? b : StringHandlers.capitalize(a[1]))));
                                }
                                else if (typeof b === "string")
                                    store2.push("\"" + StringHandlers.capitalize(a) + "\" -- " + ((b.startsWith("v")) ? b : StringHandlers.capitalize(b)));
                            }
                        }
                        if (store2.length)
                            prints = store2.join("\n");
                        else if (more[0] === "commands" && !noth)
                            prints = "\n**Nothing to show**";
                        fn = " -- " + more.map(a => StringHandlers.capitalize(a)).join(" - ");
                    }
                    else {
                        if (store.split(" ").length === 1)
                            prints = StringHandlers.capitalize(store);
                        else
                            prints = store;
                        fn = " -- " + more.map(a => StringHandlers.capitalize(a)).join(" - ");
                    }
                ...
                for (const a in ud) {
                    if (Array.isArray(ud[a])) {
                        ...
                        if (!Object.values(ud[a]).reduce((a, b) => a + Object.keys(b).length, 0))
                            continue;
                        prints.push("**" + StringHandlers.capitalize(a) + ":**");
                        for (const c in ud[a]) {
                            if (!Object.keys(ud[a][c]).length)
                                continue;
                            if (a === "other") {
                            ...
                            for (const [e, f] of Object.entries(ud[a][c])) {
                                ...
                                else if (a === "plans to update")
                                    prints.push("> \"" + e + "\" -- " + StringHandlers.capitalize(f));
                            }
                        }
                    }
                    else
                ...
                if (prints.join("\n").length > 1000) {
                    ...
                    for (const i in tlist) {
                        while (tlist[i].join("\n").length > 1000) {
                            if (!i)
                                tlist[1].unshift(tlist[0].pop());
                            else
                                tlist[0].push(tlist[1].shift());
                        }
                    }
                    ...
                    embed.addField(version, prints.join("\n"), false);
            }
        }
        else {
            const [version, ud] = Object.entries(udslist).slice(-1)[0], formver = defformver(version);
            let prints = [];
            for (const a in ud) {
                if (Array.isArray(ud[a])) {
                    if (!ud[a].length)
