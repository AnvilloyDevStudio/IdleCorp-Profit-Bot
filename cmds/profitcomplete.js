...
const Command = {
    ...
    execute(message, args, extra) {
        ...
        if (!(fac.replaceAll(" ", "_") in ICDetail["facilities"]))
            fac = Object.entries(icals["ic"]["facilities"]).find(a => (a[1]?.includes(fac)))?.[0];
        else
            fac = fac.replaceAll(" ", "_");
        if (!(fac in ICDetail["facilities"]))
            return message.channel.send("`EN0002`: Invalid facility: " + args.join(" ") + ".");
        const oriNum = num;
        num *= { "day": 60 * 60 * 24, "hour": 60 * 60, "minute": 60 }[unit] ?? 1;
        const ICDetails = JSON.parse(JSON.stringify(Object.assign({}, ICDetail.facilities, { airport: Object.assign({}, ICDetail.facilities.airport, { speed: 20 }) })));
        const facDetail = ICDetails[fac];
                        ...
                        listener.on("authorized", auth => {
                            IdleCorpConnection.getRegionalModifiers(auth.token, regionId, null).then(b => {
                                if (b === null)
                                    ...
                                    for (const f in ICDetails)
                                        if (f in b)
                                            ICDetails[f].speed = Math.round(new Decimal(b[f]).mul(ICDetails[f].speed).div(100 + b["happiness"]).toNumber());
                                        else
                                            ICDetails[f].speed = Math.round(new Decimal(ICDetails[f].speed).div(100 + b["happiness"]).toNumber());
                                }
                                rs();
                            });
                            listener.removeAllListeners();
...
export { Command };
//# sourceMappingURL=profitcomplete.js.map