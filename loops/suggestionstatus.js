...
export default (client, database) => {
    const int = setInterval(() => {
        try {
            database.query("SELECT updated FROM \"updated\" WHERE \"table\" = \'suggestions\'").then((dt) => {
                if (dt.rows[0].updated)
                    return;
                database.query("SELECT * FROM \"suggestions\";").then(dt => {
                    ...
                    if (res.reduce((a, b) => a + b.reduce((a, b) => a + b.length, 0), 0) > 5500) {
                        ...
                        for (const a of values) {
                            if ((72 + res[ind].join("\n").length) > 1000) {
                                ...
                                res.push([]);
                                if (ind > 5) {
                                    res[6].push("And " + (values.length - i - 1) + " more...");
                                    break;
                                }
                            }
                    ...
                    client.guilds.cache.get("801019800682758145").channels.cache.get("856913820978642964").messages.fetch("856914255064203274").then(msg => msg.edit({ embeds: [new Discord.MessageEmbed().setTitle("Suggestions").addFields(res.map(a => ({ name: "\u200b", value: a.join("\n"), inline: false })))] }));
...
//# sourceMappingURL=suggestionstatus.js.map