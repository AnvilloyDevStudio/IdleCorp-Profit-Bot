...
class NumberHandlers {
    ...
    static DectoInt(...a) {
        if (a.some(a => !Number.isInteger(a)))
            return this.DectoInt(...a.map(a => new Decimal(a).mul(10).toNumber()));
        return a;
    }
    static toTime(num = 0) {
        let result = [];
    ...
    static timeAlias(str, defa = 0, ret = 0) {
        ...
        if (/^[0-9]+(?:\.[0-9]+)?$/.test(str))
            return parseFloat(str) * timetable[defa] / timetable[ret];
        const [time, unit] = str.match(/^([0-9]+)([a-z]+)/i).slice(1);
        if (!["ms", "s", "sec", "m", "min", "h", "hr", "d", "w"].includes(unit) || !time)
            return false;
        return parseFloat(time) * ttable[unit] / timetable[ret];
    }
    static numberToLocaleString(num) {
        return num.toLocaleString("en-US");
    }
    static numberToLocaleStringFloating(num, digits = 2) {
        return num.toLocaleString("en-US", { minimumFractionDigits: digits });
    }
}
export { NumberHandlers };
//# sourceMappingURL=NumberHandlers.js.map